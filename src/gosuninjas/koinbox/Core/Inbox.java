...
/**
 * This class conducts the view Inbox activity, which allows users to open his/her inbox and view messages that he/she
 * has sent to other users. The class goes with myinbox.xml layout that for each message, it shows Subject and the recipient.
 * @author Cuong
 */

public class Inbox extends Activity implements OnClickListener {
	HttpClient client;
	final Context context = this;
	ProgressDialog p;
	Button frnProf, frnMKB, frnFriend, frnLogout;
	public List<String> subject_list = new ArrayList<String>();
	public List<String> sender_list = new ArrayList<String>();
	public List<String> body_list = new ArrayList<String>();
	@Override
	public void onCreate(Bundle savedInstanceState){
		super.onCreate(savedInstanceState);
		setContentView(R.layout.myinbox);
		p = ProgressDialog.show(Inbox.this, "", "Loading...");
		...
		TextView txt = (TextView) findViewById(R.id.iTitle);  
		...
		TextView pba = (TextView) findViewById(R.id.iBack); 
		 ...
		 * sender and contents to shows them in the layout.
		 ...
		 */
		final TableLayout layout = (TableLayout) findViewById(R.id.inbox_layout);
		if (subject_list.size()==0){
			TextView tx1 = new TextView(Inbox.this);
			tx1.setText("You don't have any messages in your inbox!");
			layout.addView(tx1);
		}
		else{
		...
		for (int i=0;i<subject_list.size();i++){
			tx[j]=new TextView(Inbox.this);
			tx[j+1]=new TextView(Inbox.this);
			tx[j+2]=new TextView(Inbox.this);
			tx[j+2].setText("");
			tx[j].setText("From: " +sender_list.get(i));
			...
			final String l=sender_list.get(i);
			...
			tx[j+1].setOnClickListener(new TextView.OnClickListener() {  
				   public void onClick(View v) {
					   ViewMessage.view_recipient=Koinbox.username;
					   ViewMessage.view_sender=l;
					   ViewMessage.view_subject=m;
					   ViewMessage.view_body=k;
					   Intent i = new Intent(Inbox.this, ViewMessage.class);
					   startActivity(i);
				   }	   
				   });
	 ...
	 * 
	 */
	public void getInbox() throws ClientProtocolException, IOException, JSONException{
		HttpGet get = new HttpGet("http://myapp-gosuninjas.dotcloud.com/api/v1/message/?format=json&recipient_username="+Koinbox.username);
		...
		for (int i=0;i<message_stream.length();i++){
			...
			sender_list.add(message_stream.getJSONObject(i).getString("sender"));
